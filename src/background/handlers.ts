...
import { applyReclassifyResults } from '@/core/plan'
...
import {
  PlanAnalysisError, PlanCancelledError, planAdditiveAnalysis, planTitleOnlyAnalysis,
} from './plan-analysis'
import {
...
export async function handle(
  ...
  try {
    switch (request.kind) {
      ...
      case 'analyze': {
        // 编排都在 plan-analysis.ts / rebuild.ts 里：这里只做模式判定与分发，
        // 业务错误以类型抛出、在此处统一映射成响应（与 moveBookmarks 同一条规矩）。
        const llm = activeLlm(settings)
        if (request.titleOnly === true) {
          return { ok: true, kind: 'analyze', outcome: 'plan', plan: await planTitleOnlyAnalysis({
            ports,
        ...
        try {
          const plan = await planAdditiveAnalysis({
            ports,
            scan,
            roots,
            scopeRootIds: request.scopeRootIds,
            ...
            concurrency,
            batchSize: deps.batchSize,
            onlyLoose: settings.onlyLooseInAdditive,
            rewriteGithubTitles: settings.rewriteGithubTitles,
            emit,
            isCancelled,
            now,
          ...
          if (error instanceof PlanCancelledError) return CANCELLED
          if (error instanceof PlanAnalysisError) return { ok: false, error: error.message }
          throw error
        }
      }
